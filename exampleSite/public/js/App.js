...
$(document).ready(function() {
	...
	d3.json(window.baseURL + "/js/world.json", function(error, world) {
		...
		var Start = Date.now();
		var width = $('#sideNav').outerWidth(true);
	   	var height = $('#sideNav').outerHeight(true);
		var rotate = [10];
	    var velocity = [0.001];
		...
		var svg = d3.select("#sideNav").insert("svg", "#navbarSupportedContent")
			.attr("id", "menu-globe")
			.attr("width", width)
			.attr("height", height);
		...
		function updateWindow(){
			var w = $('#sideNav').outerWidth(true);
			var h = $('#sideNav').outerHeight(true);
